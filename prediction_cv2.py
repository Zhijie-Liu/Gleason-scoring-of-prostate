...
def get_probability_matrix(net, cuda, nrow, ncol, img_mosaic):
    ...
    for i in range(0, nrow - step + 1):
        for j in range(0, ncol - step + 1):
            ...
            img_temp = img_mosaic[i * img_size: (i + 1) * img_size, j * img_size: (j + 1) * img_size]
            img = preprocess(transforms.ToPILImage()(img_temp)).unsqueeze(0)
...
if __name__ == '__main__':
    select = 0
    if select == 0:
        img_dir = 'score/zhuchenyun A-2.tif (RGB).tif'
