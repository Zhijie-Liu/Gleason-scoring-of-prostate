...
if __name__ == '__main__':
    ...
    models._modules.get('avgpool_1a').register_forward_hook(hook_feature)
    ...
    for dir in path_dir:
        ...
        for path in img_path:
            ...
            if torch.cuda.is_available():
                img = img.to(gpu)
            output = models(img)
            layer = np.squeeze(features_blobs[0],axis=(0,2,3))
            ...
            # img_name = os.path.join(os.path.basename(path).split('.')[0])
    ...
    layer_data = pd.DataFrame(data=layer_list, index=img_list)
    layer_data.T.to_csv(os.path.join(save_path, img_dir.split('/')[-1] + '_layer.csv'), index=False)
