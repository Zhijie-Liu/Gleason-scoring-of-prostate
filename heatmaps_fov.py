...
if __name__ == '__main__':
    ...
    if select == 0:
        ...
        net_path = 'date20210906162244crossvalid 20210906 two class/InceptionResNetV2params_Adamepochs120.pkl'
    elif select == 1:
    ...
    CAM_dir = os.path.dirname(img_path) + '/' + 'CAM_' + os.path.basename(img_path).split('.')[-2]
    core_lzj.check_folder_existence(CAM_dir)
    net.load_state_dict(torch.load(net_path, map_location='cuda:' + gpu.__str__()))
    ...
    img_pil.save(CAM_dir + '/Raw_' + os.path.basename(img_path))
    cv2.imwrite(CAM_dir + '/Raw+CAM_' + os.path.basename(img_path), result)
    cv2.imwrite(CAM_dir + '/CAM_' + os.path.basename(img_path), heatmap)
