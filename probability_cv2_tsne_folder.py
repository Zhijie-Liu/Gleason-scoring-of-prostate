...
def get_probability_matrix(net, cuda, nrow, ncol, img_mosaic, name):
    ...
    for i in range(0, nrow - step + 1):
        for j in range(0, ncol - step + 1):
            ...
            if output_max == 0:
                ...
                # zero += 1
                matrix_0[i:i + step, j:j + step] += 1
                name_temp = os.path.join('20210902 Score/0', name + '_' + str(i+1) + '_' + str(j+1) + '.png')
                cv2.imwrite(name_temp, img_temp)
            elif output_max == 1:
                # matrix_1[i:i + step, j:j + step] += 1
                # one += 1
                matrix_1[i:i + step, j:j + step] += 1
                name_temp = os.path.join('20210902 Score/1', name + '_' + str(i+1) + '_' + str(j+1) + '.png')
                cv2.imwrite(name_temp, img_temp)
            elif output_max == 2:
                # matrix_no[i:i + step, j:j + step] += 1
                # two += 1
                matrix_2[i:i + step, j:j + step] += 1
                name_temp = os.path.join('20210902 Score/2', name + '_' + str(i+1) + '_' + str(j+1) + '.png')
                cv2.imwrite(name_temp, img_temp)
            elif output_max == 3:
                matrix_3[i:i + step, j:j + step] += 1
                name_temp = os.path.join('20210902 Score/3', name + '_' + str(i+1) + '_' + str(j+1) + '.png')
                cv2.imwrite(name_temp, img_temp)
                # three += 1
            else:
                matrix_no[i:i + step, j:j + step] += 1
...
if __name__ == '__main__':
    select = 0
    if select == 0:
        img_dir = '20210902 Score/C1.tif (RGB).tif'
        net_path = 'last 20210610 four class/date20210614223143crossvalid 20210610/InceptionResNetV2params_Adamepochs120.pkl'
    elif select == 1:
    ...
    core_lzj.check_folder_existence('20210902 Score/0')
    core_lzj.check_folder_existence('20210902 Score/1')
    core_lzj.check_folder_existence('20210902 Score/2')
    core_lzj.check_folder_existence('20210902 Score/3')
    img_name = img_dir.split('/')[-1].split('.')[0]
    prob_matrix_0, prob_matrix_1, prob_matrix_2, prob_matrix_3, prob_matrix_no = get_probability_matrix(net=models, cuda=device, nrow=img_nrow, ncol=img_ncol, img_mosaic=img_raw, name=img_name)
    # print(percentage)
