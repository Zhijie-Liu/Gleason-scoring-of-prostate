...
if __name__ == '__main__':
    ...
    if select == 0:
        ...
        img_dir = 'low to high grade test/high grade'
        net_path = 'low to high grade test/InceptionResNetV2params_Adamepochs60.pkl'
    elif select == 1:
    ...
    img_path = core_lzj.each_img(filepath=img_dir)
    ...
    for path in img_path:
        print(path)
        heatmap_path = os.path.join(save_path, os.path.basename(path).split('.')[0])
        ...
        for i in range(0, img_nrow - step + 1):
            for j in range(0, img_ncol - step + 1):
                ...
                heatmap_name = os.path.basename(path).split('.')[0] + '_' + str(i + 1) + '_' + str(j + 1) + '_heatmap.png'
